...
async function renderCatchList() {
    ...
    try {
        ...
        // ★ 修正: orderBy を where の前に持っていく (Dexie 3.x の filter() 後は orderBy が使えないため)
        // -> いや、orderBy は filter の後でも使える。
        // -> ★ 修正: ソートキーがインデックスにあることを確認
        // db.js: catch_records: '++id, trap_id, gun_log_id, catch_date, species_name, gender, age'
        // OK, 'catch_date' と 'species_name' はインデックスにある。
