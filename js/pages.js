// ============================================================================
// js/pages.js - 各画面の描画ロジック (v2: 機能強化版)
// ============================================================================
...
async function renderTrapList() {
    ...
    try {
        ...
        listEl.innerHTML = traps.map((trap, index) => {
            ...
            // ★追加: 経過日数の計算
            let elapsedStr = '';
            if (trap.is_open === 1 && trap.setup_date) {
                const setup = new Date(trap.setup_date);
                const diffTime = Math.abs(today - setup);
                const diffDays = Math.floor(diffTime / (1000 * 60 * 60 * 24)); 
                elapsedStr = `<span class="ml-2 text-xs bg-gray-200 text-gray-700 px-1 rounded">${diffDays}日目</span>`;
            }
            ...
            return `
                <div class="trap-card flex items-center" data-id="${trap.id}">
                    <div class="flex-grow">
                        ...
                        <p class="text-sm text-gray-600">${escapeHTML(trap.type)} <span class="text-gray-400">|</span> ${formatDate(trap.setup_date)}${elapsedStr}</p>
    ...
    document.getElementById('ammo-tbody').innerHTML = combined.map(r => `
        <tr class="border-b">
            ...
            <td class="p-1 text-right ${r.a > 0 ? 'text-green-600' : 'text-red-600'}">
                ${r.a > 0 ? '+' : ''}${r.a}
                ${r.isP ? `<button class="btn btn-danger btn-sm ml-2 del-ammo" data-id="${r.id}">×</button>` : ''}
            </td>
        </tr>
    `).join('');
...
async function showGunEditForm(id) {
    ...
    document.getElementById('gun-form').onsubmit = async (e) => {
        ...
        try {
            if(id) { await db.gun.update(id, data); showToast('更新しました', 'success'); showGunDetailPage(id); }
            else { const nid = await db.gun.add(data); showToast('登録しました', 'success'); showGunDetailPage(nid); }
        } catch { showToast('保存失敗', 'error'); }
    };
}
...
async function showCatchEditForm(id, { trapId, gunLogId }) {
    let r = { catch_date: new Date().toISOString().split('T')[0], species_name: '', weight: '', gender: '不明', age: '不明', latitude: '', longitude: '', memo: '', image_blob: null, trap_id: null, gun_log_id: null };
    ...
    if(document.getElementById('unlink-btn')) document.getElementById('unlink-btn').onclick = function() {
        if(confirm('紐付けを解除しますか？')) {
            r.trap_id = null; r.gun_log_id = null;
...
async function renderChecklistSets() {
    ...
    document.getElementById('add-set-form').onsubmit = async (e) => {
        e.preventDefault();
        try { await db.checklist_sets.add({name: document.getElementById('new-set-name').value}); renderChecklistSets(); } catch { showToast('作成失敗', 'error'); }
    };
}
...
async function showChecklistDetail(listId) {
    ...
    const renderEdit = async () => {
        ...
        document.getElementById('add-item-form').onsubmit = async (e) => {
            e.preventDefault();
            try { await db.checklist_items.add({list_id: listId, name: document.getElementById('new-item-name').value, is_checked: false}); renderEdit(); } catch { showToast('追加失敗', 'error'); }
        };
...
function renderInfoTopPage() {
    ...
    app.innerHTML = `
        <div class="space-y-2">
            ...
            <div class="card bg-white hover:bg-gray-50 cursor-pointer p-4 flex items-center" onclick="showSafetyInfoPage()">
                <div class="bg-red-100 p-2 rounded-full mr-4"><i class="fas fa-exclamation-triangle text-red-600 text-2xl"></i></div>
                <div><h2 class="text-lg font-bold">捕獲時の注意</h2><p class="text-sm text-gray-500">事故防止・個体管理・環境</p></div>
                <i class="fas fa-chevron-right ml-auto text-gray-400"></i>
            </div>
        </div>
    `;
...
// 安全管理詳細
function showSafetyInfoPage() {
    updateHeader('捕獲時の注意', true);
    backButton.onclick = () => showInfoPage();

    app.innerHTML = `
        <div class="space-y-4 pb-10">
            <div class="card bg-white border-l-4 border-red-500">
                <h2 class="text-xl font-bold mb-3 text-gray-800">1. 絶対的な安全確認の徹底</h2>
                <p class="text-sm text-gray-600 mb-4">狩猟事故の多くは「確認不足」と「慣れ」から生じます。以下の基本を常に意識してください。</p>
                <h3 class="font-bold text-red-600 mb-1">■ 矢先の確認</h3>
                <p class="text-sm mb-3 pl-2 border-l-2 border-gray-200">獲物だけでなくバックストップ（安土）を必ず確認してください。背景が道路・人家である場所での発砲は犯罪です。</p>
                <h3 class="font-bold text-red-600 mb-1">■ 脱包と銃口管理</h3>
                <p class="text-sm mb-3 pl-2 border-l-2 border-gray-200">銃に弾を装填してよいのは、目の前に獲物がいる時だけです。銃口は常に安全な方向へ向けてください。</p>
                <h3 class="font-bold text-red-600 mb-1">■ 捕獲個体の逆襲</h3>
                <p class="text-sm pl-2 border-l-2 border-gray-200">発砲時は、獲物が確実に息絶えたか双眼鏡等で確認しましょう。罠の確認時も双眼鏡を使用し、見通しの効く場所へ罠をかけましょう。くくり罠のワイヤーは可能な限り短くし、斜面を避け、丈夫な生木に架設しましょう。</p>
            </div>
            <div class="card bg-white border-l-4 border-orange-500">
                <h2 class="text-xl font-bold mb-3 text-gray-800">2. 被害対策としての個体選別</h2>
                <p class="text-sm text-gray-600 mb-4">有害鳥獣捕獲や個体数調整においては、単に数を減らすだけでなく「どの個体を獲るか」が重要です。</p>
                <h3 class="font-bold text-orange-700 mb-1">■ 加害個体の特定</h3>
                <p class="text-sm mb-3 pl-2 border-l-2 border-gray-200">山奥に生息する個体よりも、農地周辺に出没し餌付いている個体を優先して捕獲することが、農林水産被害の軽減に直結します。加害種の特定に加え、加害個体の選択を意識しましょう。被害作物を意識した誘引餌や架設地点を選択しましょう。</p>
                <h3 class="font-bold text-orange-700 mb-1">■ 捕獲の時期</h3>
                <p class="text-sm pl-2 border-l-2 border-gray-200">加害個体は季節移動等で入れ替わる事があります。冬季の捕獲は山野の食料が乏しいために成功しやすく、繁殖前に加害個体群を除去できる可能性があります。被害が出る前に、被害を出す可能性が高い個体を狙って、予防的な捕獲と個体群の管理を目指しましょう。</p>
            </div>
            <div class="card bg-white border-l-4 border-green-500">
                <h2 class="text-xl font-bold mb-3 text-gray-800">3. 鉛中毒のリスクと非鉛弾の推奨</h2>
                <div class="text-sm text-gray-700 leading-relaxed">
                    <p class="mb-2">鉛弾が獲物の体内で砕けると、微細な破片となって肉や内臓に残存することがあります。これらが残滓（ざんし）として野山に残されると、猛禽類等が食べて鉛中毒を引き起こします。</p>
                    <p class="mb-2">鉛製の散弾では、カモ類で同様の問題が発生します。</p>
                    <p>北海道では既に鉛弾の使用が禁止されており、本州以南でも非鉛弾（銅弾など）への移行が強く推奨されています。生態系を守るため、回収が確実な状況や射撃場以外では非鉛の弾頭の使用を積極的に検討しましょう。</p>
                </div>
            </div>
        </div>
    `;
...
async function showHunterProfileEdit() {
    ...
    fields.forEach(f => {
        ...
        imgs.forEach(i => {
            ...
            imgHtml += `<div class="relative inline-block mr-2 mb-2"><img src="${u}" class="h-16 w-16 object-cover rounded border"><button class="absolute -top-2 -right-2 bg-red-600 text-white rounded-full w-5 h-5 text-xs del-p-img" data-id="${i.id}">×</button></div>`;
        });
...
async function renderSettingsMenu() {
    ...
    app.innerHTML = `
        <div class="space-y-2">
            <div class="card bg-white">
                ...
                <div class="space-y-0 divide-y divide-gray-100">
                    ...
                    ${helpItem('📖', '情報・図鑑', `
                        ...
                        <li><b>捕獲時の注意:</b> 狩猟事故防止や鉛弾の問題など、安全管理に関する重要事項を確認できます。</li>
                    `)}
                    ${helpItem('💾', 'バックアップ', `
            ...
            <div class="card bg-white">
                ...
                <h3 class="text-sm font-bold text-gray-500 mb-1">CSV出力・マップ</h3>
                <div class="grid grid-cols-2 gap-2 mb-2">
                ...
                <button id="exp-kml" class="btn btn-secondary w-full mb-2"><i class="fas fa-map"></i> KML(マップ)出力</button>
    ...
    // ★追加: KMLボタン処理
    document.getElementById('exp-kml').onclick = exportAllDataAsKML;
    ...
    document.getElementById('upd-dic').onclick = async () => {
        if(confirm('図鑑データをサーバー(GitHub)から取得して更新しますか？')) {
            ...
            try { await populateGameAnimalListIfNeeded(true); document.getElementById('csv-st').textContent = '完了'; showToast('更新しました', 'success'); } 
            catch { document.getElementById('csv-st').textContent = '失敗'; showToast('更新失敗', 'error'); }
        }
    };
